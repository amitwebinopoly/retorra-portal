...
class SampleController extends Controller {
    ...
    public function api_get_sample_list(){
        $InexController = new InexController();
    ...
    public function api_get_sample_doc_list($sequence_num){
        if( (isset($sequence_num))&&(!empty($sequence_num)) ){
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://retorra.ezofficeinventory.com/assets/'.$sequence_num.'.api?show_document_urls=true',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
                CURLOPT_FOLLOWLOCATION => true,
